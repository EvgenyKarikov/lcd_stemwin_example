...
void st7789_write_cmd(uint8_t cmd)
{
    ...
    HAL_SPI_Transmit(&st7789_spi_handler, &cmd, 1,100);
...
void st7789_write_data(uint8_t data)
{
    ...
    st7789_pin_write(LCD_CS_GPIO_Port, LCD_CS_Pin, GPIO_PIN_RESET);
    HAL_SPI_Transmit(&st7789_spi_handler, &data, 1,100);
    st7789_pin_write(LCD_CS_GPIO_Port, LCD_CS_Pin, GPIO_PIN_SET);
}

void st7789_write_half_word(const uint16_t data)
{
    uint8_t buf[2] = {data >> 8, data};
    st7789_pin_write(LCD_DC_GPIO_Port, LCD_DC_Pin, GPIO_PIN_SET);
    st7789_pin_write(LCD_CS_GPIO_Port, LCD_CS_Pin, GPIO_PIN_RESET);
    HAL_SPI_Transmit(&st7789_spi_handler, buf, 2,100);
    st7789_pin_write(LCD_CS_GPIO_Port, LCD_CS_Pin, GPIO_PIN_SET);
}
...
void st7789_transfer_datas_no_dma(uint8_t buffer[], uint32_t bufsize)
{
    uint8_t *pbuf = buffer;
    ...
    HAL_SPI_Transmit(&st7789_spi_handler, pbuf, bufsize,1000);
...
void st7789_fill_color(uint16_t color)
{
...
#if (0)
    static uint8_t buf[ST7789_FILL_COLOR_SIZE] = {0};     
    ...
    for (uint16_t i = 0; i < ST7789_H; ++i)
        for (uint16_t j = 0; j < ST7789_W; ++j)
            HAL_SPI_Transmit(&st7789_spi_handler, data, 2,100);
#endif
    // Unselect SPI CS pin
    st7789_pin_write(LCD_CS_GPIO_Port, LCD_CS_Pin, GPIO_PIN_SET);
}
